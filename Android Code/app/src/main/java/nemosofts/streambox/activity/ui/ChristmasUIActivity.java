...
public class ChristmasUIActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        progressDialog = new NSoftsProgressDialog(ChristmasUIActivity.this);
        ...
        if (!ApplicationUtil.isTvBox(this)){
            new GDPRChecker(ChristmasUIActivity.this).check();
        ...
        new Handler().postDelayed(() -> {
            if (!isFinishing()){
                DialogUtil.PopupAdsDialog(ChristmasUIActivity.this);
            }
        }, 600);
    }
    ...
    private void setListenerHome() {
        ...
        findViewById(R.id.select_live).setOnLongClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_TV).isEmpty()) {
                DialogUtil.DownloadDataDialog(ChristmasUIActivity.this, Callback.TAG_TV, type -> getLive());
            }
            return false;
        });
        findViewById(R.id.select_movie).setOnLongClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
                DialogUtil.DownloadDataDialog(ChristmasUIActivity.this, Callback.TAG_TV, type -> getMovies());
            }
            return false;
        });
        findViewById(R.id.select_serials).setOnLongClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
                DialogUtil.DownloadDataDialog(ChristmasUIActivity.this, Callback.TAG_TV, type -> getSeries());
            }
            return false;
        });
    }
    ...
    public void onClick(@NonNull View id) {
        switch (id.getId()) {
            case R.id.iv_notifications ->
                    startActivity(new Intent(ChristmasUIActivity.this, NotificationsActivity.class));
            case R.id.iv_file_download ->
                    startActivity(new Intent(ChristmasUIActivity.this, DownloadActivity.class));
            case R.id.iv_radio -> {
                if (isDownloadLive()) {
                    startActivity(new Intent(ChristmasUIActivity.this, RadioActivity.class));
                }
            }
            case R.id.iv_profile ->
                    startActivity(new Intent(ChristmasUIActivity.this, ProfileActivity.class));
            case R.id.iv_profile_re -> sign_out();
            case R.id.iv_settings ->
                    startActivity(new Intent(ChristmasUIActivity.this, SettingActivity.class));
            case R.id.select_live -> {
                if (spHelper.getCurrent(Callback.TAG_TV).isEmpty()) {
                    getLive();
                } else {
                    startActivity(new Intent(ChristmasUIActivity.this, LiveTvActivity.class));
                }
            }
            case R.id.select_movie -> {
                if (spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
                    ...
                    startActivity(new Intent(ChristmasUIActivity.this, MovieActivity.class));
                }
            }
            case R.id.select_serials -> {
                if (spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
                    ...
                    startActivity(new Intent(ChristmasUIActivity.this, SeriesActivity.class));
                }
            }
            case R.id.select_epg -> {
                if (isDownloadLive()) {
                    startActivity(new Intent(ChristmasUIActivity.this, CategoriesActivity.class));
                }
            }
            case R.id.select_multiple_screen -> {
                if (isDownloadLive()) {
                    startActivity(new Intent(ChristmasUIActivity.this, MultipleScreenActivity.class));
                }
            }
            case R.id.select_catch_up -> {
                if (isDownloadLive()) {
                    startActivity(new Intent(ChristmasUIActivity.this, CatchUpActivity.class));
                }
            }
            ...
            case R.id.ll_movie_auto_renew -> {
                if (!spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
                    DialogUtil.DownloadDataDialog(ChristmasUIActivity.this, Callback.TAG_MOVIE, type -> getMovies());
                } else {
                    getMovies();
                }
            }
            case R.id.ll_series_auto_renew -> {
                if (!spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
                    DialogUtil.DownloadDataDialog(ChristmasUIActivity.this, Callback.TAG_SERIES, type -> getSeries());
                } else {
                    getSeries();
                }
            }
    ...
    private void sign_out() {
        DialogUtil.LogoutDialog(ChristmasUIActivity.this, () -> {
            Intent intent = new Intent(ChristmasUIActivity.this, UsersListActivity.class);
            if (spHelper.isLogged()) {
                new JSHelper(ChristmasUIActivity.this).removeAllData();
                dbHelper.removeAllData();
                spHelper.removeSignOut();
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                intent.putExtra("from", "");
                Toast.makeText(ChristmasUIActivity.this, getString(R.string.logout_success), Toast.LENGTH_SHORT).show();
            } else {
    ...
    private void getSeries() {
        if (NetworkUtils.isConnected(this)){
                ...
                public void onEnd(String success, String msg) {
                    ...
                    if (!isFinishing()){
                        if (success.equals("1")) {
                            ...
                            spHelper.setCurrentDate(Callback.TAG_SERIES);
                            changeIcon(spHelper.getCurrent(Callback.TAG_SERIES).isEmpty(), Callback.TAG_SERIES,false);
                            Toast.makeText(ChristmasUIActivity.this, getString(R.string.added_success), Toast.LENGTH_SHORT).show();
                        }  else {
                            spHelper.setCurrentDateEmpty(Callback.TAG_SERIES);
                            changeIcon(spHelper.getCurrent(Callback.TAG_SERIES).isEmpty(), Callback.TAG_SERIES,true);
                            pb_serials.setVisibility(View.GONE);
                            if (success.equals("3")){
                                Toasty.makeText(ChristmasUIActivity.this, msg, Toasty.ERROR);
                            } else {
                                Toast.makeText(ChristmasUIActivity.this, getString(R.string.err_server_not_connected), Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
            ...
            pb_serials.setVisibility(View.GONE);
            Toasty.makeText(ChristmasUIActivity.this, getString(R.string.err_internet_not_connected), Toasty.ERROR);
        }
    }
    ...
    public void onBackPressed() {
        if (ApplicationUtil.isTvBox(ChristmasUIActivity.this)) {
            super.onBackPressed();
        } else {
            DialogUtil.ExitDialog(ChristmasUIActivity.this);
        }
    }
