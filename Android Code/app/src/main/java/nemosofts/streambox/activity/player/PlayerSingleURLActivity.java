...
public class PlayerSingleURLActivity extends AppCompatActivity {
    ...
    private class PlayerListener implements Player.Listener {
        ...
        public void onPlayerError(@NonNull PlaybackException error) {
            ...
            if (!isFinishing()){
                if (playback < 5){
                    playback = playback + 1;
                    Toast.makeText(PlayerSingleURLActivity.this,"Playback error - "+ String.valueOf(playback)+"/5 ", Toast.LENGTH_SHORT).show();
    ...
    private void startTimer() {
        if (!isTvBox && countDownTimer == null && Callback.reward_ad_on_single){
            countDownTimer = new CountDownTimer(timeLeftInMillis, 1000) {
                ...
                public void onFinish() {
                    if (!isFinishing()){
                        helper.showRewardAds(Callback.reward_ad_on_single,exoPlayer != null && exoPlayer.isPlaying(), playWhenReady1 -> {
                        });
                    }
                }
            }.start();
