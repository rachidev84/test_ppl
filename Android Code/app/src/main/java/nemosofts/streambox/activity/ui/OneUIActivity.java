...
public class OneUIActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!ApplicationUtil.isTvBox(this)){
            new GDPRChecker(OneUIActivity.this).check();
        ...
        new Handler().postDelayed(() -> {
            if (!isFinishing()){
                DialogUtil.PopupAdsDialog(OneUIActivity.this);
            }
        }, 600);
    }
    ...
    public void onClick(@NonNull View id) {
        switch (id.getId()) {
            ...
            case R.id.ll_movie_auto_renew -> {
                if (!spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
                    DialogUtil.DownloadDataDialog(OneUIActivity.this, Callback.TAG_MOVIE, type -> getMovies());
                } else {
                    getMovies();
                }
            }
            case R.id.ll_series_auto_renew -> {
                if (!spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
                    DialogUtil.DownloadDataDialog(OneUIActivity.this, Callback.TAG_SERIES, type -> getSeries());
                } else {
                    getSeries();
                }
            }
